...
# ---- Step 2: Bootstrap (install deps + generate files) ----
# Pass "native" to avoid WASM/Android SDK installs — we handle those ourselves
echo ">>> Running bootstrap.sh native..."
bash bootstrap.sh native 2>&1 | tail -10 || echo "  (bootstrap partial — continuing)"
...
# ---- Step 6c: Generate protobuf C++ headers ----
echo ">>> Generating protobuf C++ headers..."
# Use wallet-core's bundled protoc (matches protobuf 3.20.3) to avoid version mismatch
LOCAL_PROTOC="$WC_DIR/build/local/bin/protoc"
if [ ! -f "$LOCAL_PROTOC" ]; then
  LOCAL_PROTOC=$(which protoc)
  echo "  Warning: using system protoc ($LOCAL_PROTOC) — may cause version mismatch"
fi
for proto in "$WC_DIR"/src/proto/*.proto; do
  [ -f "$proto" ] && "$LOCAL_PROTOC" --cpp_out="$WC_DIR/src/proto" -I "$WC_DIR/src/proto" "$proto" 2>/dev/null
done
# Tron internal proto
for proto in "$WC_DIR"/src/Tron/Protobuf/*.proto; do
  [ -f "$proto" ] && "$LOCAL_PROTOC" --cpp_out="$(dirname "$proto")" -I "$(dirname "$proto")" "$proto" 2>/dev/null
done
